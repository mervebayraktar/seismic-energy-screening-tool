(function () {
  ...
  function importRawObjects(rawObjects) {
    ...
    for (const raw of rawObjects) {
      const record = mapImportedRow(raw);
      if (!record) continue;
  ...
  function mapImportedRow(raw) {
    const mappedBs = {
      ...
      soil_class: raw["Soil class"],
      sds_band: raw["Earthquake Risk"],
      structural_system: raw["What is the structural system?"],
      project_year_band: raw["What is the year of the building project?"],
      storeys: raw["Number of storeys of the building?"],
      vertical_irregularity: raw["Vertical irregularity?"],
      plan_irregularity: raw["Plan irregularity?"],
      heavy_overhang: raw["Heavy overhang?"],
      layout_type: raw["What is the building layout type ?"],
      site_slope: raw["Site slope?"],
      floor_diaphragm: raw["Floor diaphragm effect?"],
      short_columns: raw["Short columns?"],
      weak_storey: raw["Weak Storey?"],
    };

    const bsMissing = validateFields(BS_FIELDS.filter((field) => field.key !== "building_label"), mappedBs);
    if (bsMissing.length) return null;
    ...
    const eesValues = {
      roof_type: raw["Roof suitable for solar panels"],
      roof_shading: raw["For roof shading condition"],
      roof_insulation: raw["Roof insulation TS 825"],
      ceiling_bracing: raw["Suspended ceiling bracing system"],
      ceiling_mep: raw["MEP system in suspended ceiling"],
      ventilation_type: raw["What is the type of boiler used for the ventilation system?"],
      ventilation_age: raw["Age/efficiency of ventilation"],
      facade_cladding: raw["Facade cladding dead load & collapse risk"],
      facade_insulation: raw["Facade insulation TS 825"],
      facade_cracks: raw["Cracks on the facade"],
      heating_fuel: raw["Primary heating fuel"],
      heating_typology: raw["Heating system typology"],
      boiler_age: raw["Age/efficiency of boiler"],
      partition_insulation: raw["Do partition walls require insulation"],
    };
    ...
    if (risk_level !== "red") {
      ...
      if (!eesMissing.length) {
        eesScore = scoreRecord(eesValues, EES_FIELDS).total;
      }
    }
  ...
  function loadDemoData() {
    ...
    recomputePortfolio();
    savePortfolio();
    els.batchStatus.textContent = `Loaded ${imported.length} sample records into the browser portfolio.`;
  }

  // Builds the flat array of row objects (one per building) shared by the CSV and
  // XLSX export paths, using the same header names as the original workbook so the
  // downloaded file stays compatible with it.
  function buildExportRows() {
    return portfolio.map((row) => ({
      "Building Name": row.building_label,
      "Soil class": row.soil_class,
      "Earthquake Risk": row.sds_band,
      "What is the structural system?": row.structural_system,
      "What is the year of the building project?": row.project_year_band,
      "Number of storeys of the building?": row.storeys,
      "Vertical irregularity?": row.vertical_irregularity,
      "Plan irregularity?": row.plan_irregularity,
      "Heavy overhang?": row.heavy_overhang,
      "What is the building layout type ?": row.layout_type,
      "Site slope?": row.site_slope,
      "Floor diaphragm effect?": row.floor_diaphragm,
      "Short columns?": row.short_columns,
      "Weak Storey?": row.weak_storey,
      Score: row.score,
      "Risk Level": row.risk_level,
      "What Should Do ?": row.action,
      "Included or Excluded": statusFromColor(row.risk_level),
      "Roof suitable for solar panels": row.roof_type,
      "For roof shading condition": row.roof_shading,
      "Roof insulation TS 825": row.roof_insulation,
      "Suspended ceiling bracing system": row.ceiling_bracing,
      "MEP system in suspended ceiling": row.ceiling_mep,
      "What is the type of boiler used for the ventilation system?": row.ventilation_type,
      "Age/efficiency of ventilation": row.ventilation_age,
      "Facade cladding dead load & collapse risk": row.facade_cladding,
      "Facade insulation TS 825": row.facade_insulation,
      "Cracks on the facade": row.facade_cracks,
      "Primary heating fuel": row.heating_fuel,
      "Heating system typology": row.heating_typology,
      "Age/efficiency of boiler": row.boiler_age,
      "Do partition walls require insulation": row.partition_insulation,
      "Energy Efficiency Score (EES)": row.ees_score,
  ...
  function downloadPortfolioCsv() {
    const headers = [
      ...
      "What is the year of the building project?",
      "Number of storeys of the building?",
      "Vertical irregularity?",
      "Plan irregularity?",
      "Heavy overhang?",
      "What is the building layout type ?",
      "Site slope?",
      ...
      "For roof shading condition",
      "Roof insulation TS 825",
      "Suspended ceiling bracing system",
      "MEP system in suspended ceiling",
      "What is the type of boiler used for the ventilation system?",
      "Age/efficiency of ventilation",
      "Facade cladding dead load & collapse risk",
      "Facade insulation TS 825",
      "Cracks on the facade",
      "Primary heating fuel",
      "Heating system typology",
      "Age/efficiency of boiler",
      "Do partition walls require insulation",
    ...
    const lines = [headers].concat(
      portfolio.map((row) => [
        ...
        row.soil_class,
        row.sds_band,
        row.structural_system,
        row.project_year_band,
        row.storeys,
        row.vertical_irregularity,
        row.plan_irregularity,
        row.heavy_overhang,
        row.layout_type,
        row.site_slope,
        row.floor_diaphragm,
        ...
        row.risk_level,
        row.action,
        statusFromColor(row.risk_level),
        row.roof_type,
        row.roof_shading,
        row.roof_insulation,
        row.ceiling_bracing,
        ...
        row.ventilation_age,
        row.facade_cladding,
        row.facade_insulation,
        row.facade_cracks,
        row.heating_fuel,
        row.heating_typology,
        row.boiler_age,
        row.partition_insulation,
        row.ees_score,
        row.prioritization,
      ])
    );
  ...
  function bindEvents() {
    ...
    els.importCsvBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        if (isXlsxFile(file)) {
          ...
          count = importXlsxArrayBuffer(buffer);
        } else {
          const text = await file.text();
          count = importRowsFromCsvRows(parseCsv(text));
        }
        els.batchStatus.textContent = `Imported ${count} records from ${file.name}.`;
      } catch (err) {
    ...
    els.importPastedCsvBtn.addEventListener("click", () => {
      ...
      if (!text) return;
      const count = importRowsFromCsvRows(parseCsv(text));
      els.batchStatus.textContent = `Imported ${count} records from pasted CSV text.`;
    });
  ...
  function renderExpectedHeaders() {
    ...
    els.expectedHeaders.innerHTML = `<div class="small">${headers.map((header) => `• ${escapeHtml(header)}`).join("<br />")}</div>`;
  }
